...
diamante_gwas <- read_tsv(
  ...
  dplyr::select(varID=VAR_ID, p=P_VALUE, N=N_PH) %>%
  filter(p < 5e-8) %>%
...
exchip_gwas <- read_tsv(
  ...
  dplyr::select(varID=VAR_ID, p=P_VALUE, N=Neff) %>%
  filter(p < 5e-8) %>%
  separate(varID, into=c("chr", "pos", "EA", "NEA"), sep="_", remove=F)
...
mahajan_gwas <- read_tsv(
  ...
  dplyr::select(varID=SNP, chr=Chr, pos=Pos, p=Pvalue, N=Neff, EA, NEA) %>%
...
t2d_eur_gwas <- read_tsv(
  ...
  dplyr::select(varID=SNP, chr=CHR, pos=BP, p=Pvalue, N=Neff, 
